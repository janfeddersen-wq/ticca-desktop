...
fn main() {
    ...
    let default_context_window = context_windows
        .get("_default")
        .copied()
        .unwrap_or(100_000);
...
fn load_context_overrides(manifest_dir: &str) -> HashMap<String, u64> {
    ...
    let value: serde_json::Value = match serde_json::from_str(&content) {
        Ok(v) => v,
        Err(e) => {
            println!("cargo:warning=Could not parse context_overrides.json: {}", e);
            return HashMap::new();
        }
    };
    ...
    if !map.is_empty() {
        println!("cargo:warning=Loaded {} context window overrides", map.len());
    }
...
fn generate_registry_code(
    ...
    .unwrap();
    writeln!(file, "// Generated from data/context_windows.json and data/api_cache.json").unwrap();
    writeln!(file).unwrap();

    // Generate providers function
    writeln!(file, "/// Generated provider definitions from models.dev API").unwrap();
    writeln!(file, "pub fn generated_providers() -> Vec<ProviderDefinition> {{").unwrap();
    ...
    for provider in api_data {
        ...
        writeln!(file, "            name: \"{}\".to_string(),", escape_str(&provider.name)).unwrap();
        writeln!(
            file,
        ...
        writeln!(file, "            is_openai_compatible: {},", is_openai_compatible).unwrap();
    ...
    for provider in api_data {
        for model in &provider.models {
            ...
            writeln!(file, "            provider_id: \"{}\".to_string(),", provider.id).unwrap();
            writeln!(file, "            name: \"{}\".to_string(),", escape_str(&model.name)).unwrap();
            writeln!(file, "            family: {},", family).unwrap();
            writeln!(file, "            context_window: {},", context_window).unwrap();
            writeln!(file, "            max_output_tokens: Some({}),", max_output_tokens).unwrap();
    ...
    writeln!(file, "pub fn generated_context_windows() -> std::collections::HashMap<&'static str, u64> {{").unwrap();
    ...
    for (model_id, context_window) in context_windows {
        ...
        writeln!(file, "    map.insert(\"{}\", {});" , model_id, context_window).unwrap();
    }
    ...
    writeln!(file).unwrap();
    writeln!(file, "/// Context window overrides (provider:model_id or model_id -> tokens)").unwrap();
    writeln!(file, "/// These take precedence over API data for specific provider configurations").unwrap();
    writeln!(file, "pub fn generated_context_overrides() -> std::collections::HashMap<&'static str, u64> {{").unwrap();
    writeln!(file, "    let mut map = std::collections::HashMap::new();").unwrap();
    ...
    println!("cargo:warning=Generated registry_generated.rs with {} providers and {} models",
        api_data.len(),
