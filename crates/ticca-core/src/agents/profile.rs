...
use super::{AgentRegistry, AgentType, ToolPolicyType, get_agent};
...
impl AccountSelectionPolicy {
    pub fn for_agent(agent_type: AgentType) -> Self {
        ...
        Self {
            provider_order: metadata.provider_order.to_vec(),
            cooldown_aware: true,
        }
    }
...
impl AgentProfile {
    pub fn for_type(agent_type: AgentType, max_tool_rounds: u32) -> Self {
        ...
        let metadata = AgentRegistry::get(agent_type);
        let tool_usage_policy = match metadata.tool_policy {
            ToolPolicyType::FullAccess => ToolUsagePolicy::coding(),
            ToolPolicyType::ReadOnly => ToolUsagePolicy::planning(),
        };
