...
fn generate_diff(old_content: &str, new_content: &str, file_path: &str) -> String {
    ...
    while i < old_lines.len() || j < new_lines.len() {
        if i < old_lines.len() && j < new_lines.len() && old_lines[i] == new_lines[j] {
            ...
            i += 1;
            j += 1;
        } else if i < old_lines.len() && (j >= new_lines.len() || !new_lines.contains(&old_lines[i])) {
            diff_lines.push(format!("-{}", old_lines[i]));
            i += 1;
...
fn edit_with_content(payload: &ContentPayload) -> Result<ToolResult> {
    ...
    let diff = if old_content.is_empty() {
        format!("Created new file: {} ({} bytes)", payload.file_path, payload.content.len())
    } else {
    ...
    Ok(ToolResult::success(format!(
        "File '{}' {} successfully.\n\n{}",
        payload.file_path,
        if old_content.is_empty() { "created" } else { "overwritten" },
        diff
    )))
}
...
fn edit_with_replacements(payload: &ReplacementsPayload) -> Result<ToolResult> {
    ...
    if !path.exists() {
        return Ok(ToolResult::error(format!("File does not exist: {}", payload.file_path)));
    }
...
mod tests {
    ...
    fn test_replacements() {
        ...
        assert!(result.success);
        assert_eq!(fs::read_to_string(&file_path).unwrap(), "Hi, World! Hi, Rust!");
    }
    ...
    fn test_delete_snippet() {
        ...
        assert!(result.success);
        assert_eq!(fs::read_to_string(&file_path).unwrap(), "Keep this. Keep this too.");
    }
