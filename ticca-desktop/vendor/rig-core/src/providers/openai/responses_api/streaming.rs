                        ...
                        if let StreamingCompletionChunk::Delta(chunk) = &data {
                            match &chunk.data {
                                ...
                                ItemChunkKind::OutputTextDone(done) => {
                                    if !saw_text_delta && !done.text.is_empty() {
                                        combined_text.push_str(&done.text);
                                        yield Ok(streaming::RawStreamingChoice::Message(done.text.clone()))
                                    }
                                }
                                ...
                                ItemChunkKind::ContentPartDone(part) => {
                                    if !saw_text_delta {
                                        if let ContentPartChunkPart::OutputText { text } = &part.part {
                                            if !text.is_empty() {
